...
function signupTokenFunc (func, next) {
  this.signupToken = null
//  return next();
  var self = this
    , token = this.signupToken = this.param('token');
  if (token === 'wejointherevolution')
    return next();
  if (! token)
    return this.render('need_token');
  redis[func]('volunteerfeed_signup_tokens', token, function(err, success) {
    if (err)
      return next(err);
    if (!(success*1))
      return self.render('need_token');
    next();
  });
}

module.exports.signupToken = function (next) {
  signupTokenFunc.call(this, 'hexists', next);
}
module.exports.signupTokenRemove = function (next) {
  signupTokenFunc.call(this, 'hdel', next);
}
